#!/bin/bash
#SBATCH -J PGS_array_center
#SBATCH -o PGS_array_center.o%j
#SBATCH -e PGS_array_center.o%j
#SBATCH -p normal
#SBATCH -N 1
#SBATCH -n 4
#SBATCH -t 8:00:00
...
do
  ...
  do
    ...
    $plink2 \
      --bfile ${scratch}/pgs/CT_array_center_merged \
      --extract data/pgs/${phenotype}_array_center_threshold_${threshold}.txt \
      --score data/gwas_results/${phenotype}_array_center_combined.glm.linear 3 6 9 header no-mean-imputation \
      --memory 70000 \
      --out data/pgs/${phenotype}_array_center_${threshold}_scores
  done
done
