#!/bin/bash
#SBATCH -J PGS_regenie
#SBATCH -o PGS_regenie.o%j
#SBATCH -e PGS_regenie.o%j
#SBATCH -p normal
#SBATCH -N 1
#SBATCH -n 4
#SBATCH -t 8:00:00
...
do
  ...
  do
    ...
    $plink2 \
      --bfile ${scratch}/pgs/CT_regenie_merged \
      --extract data/pgs/${phenotype}_regenie_threshold_${threshold}.txt \
      --score data/gwas_results/${phenotype}_regenie_combined.glm.linear 3 6 9 header no-mean-imputation \
      --memory 70000 \
      --out data/pgs/${phenotype}_regenie_${threshold}_scores
  done
done
